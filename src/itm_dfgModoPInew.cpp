...
#include "itm_dfgModoPInew.h"

static CLxItemType gItemType_dfgModoPInew(SERVER_NAME_dfgModoPInew);
...
{
  char s[256];
  sprintf(s, "SurfDef::Prepare()  _user_channels.size() = %ld", _user_channels.size());
  feLog(s);
}
...
//  static int i = 0;
//  char s[256];
//  sprintf(s, "SurfDef::Evaluate(1) beep! %ld", i++);
//  feLog(s);
//}
...
//  static int i = 0;
//  char s[256];
//  sprintf(s, "SurfDef::Evaluate(2) beep! %ld", i++);
//  feLog(s);
//}
...
class SurfElement : public CLxImpl_TableauSurface, public CLxImpl_StringTag
{
    public:
        static void initialize ()
        {
            ...
            lx::AddSpawner            (SERVER_NAME_dfgModoPInew ".elmt", srv);
        }
...
class Surface : public CLxImpl_Surface
{
    public:
        static void initialize ()
        {
            ...
            lx::AddSpawner            (SERVER_NAME_dfgModoPInew ".surf", srv);
        }
...
LxResult Surface::surf_BinByIndex (unsigned int index, void **ppvObj)
{
    ...
    CLxSpawner<SurfElement>     spawner (SERVER_NAME_dfgModoPInew ".elmt");
...
class SurfInst : public CLxImpl_Instanceable
{
    public:
        static void initialize ()
        {
            ...
            lx::AddSpawner            (SERVER_NAME_dfgModoPInew ".instObj", srv);
        }
...
LxResult SurfInst::instable_GetSurface (void **ppvObj)
{
    ...
    CLxSpawner<Surface>     spawner (SERVER_NAME_dfgModoPInew ".surf");
...
int SurfInst::instable_Compare (ILxUnknownID other_obj)
{
    ...
    CLxSpawner<SurfInst>     spawner (SERVER_NAME_dfgModoPInew ".instObj");
    SurfInst        *other = NULL;
...
class Instance : public CLxImpl_PackageInstance, public CLxImpl_SurfaceItem, public CLxImpl_ViewItem3D
{
    public:
        static void initialize ()
        {
            ...
            lx::AddSpawner            (SERVER_NAME_dfgModoPInew ".inst", srv);
        }
    
        Instance () : _surf_spawn (SERVER_NAME_dfgModoPInew ".surf") {}
    ...
    
{
  static int i = 0;
  char s[256];
  sprintf(s, "Instance::isurf_Evaluate() beep! %ld", i++);
  feLog(s);
}
...
class Package : public CLxImpl_Package, public CLxImpl_SceneItemListener
{
    public:
        static void initialize ()
        {
            ...
            lx::AddServer            (SERVER_NAME_dfgModoPInew, srv);
        }

        Package () : _inst_spawn (SERVER_NAME_dfgModoPInew ".inst") {}
...
void Package::sil_ItemAddChannel (ILxUnknownID item_obj)
{
    ...
    if (item.test () && item.IsA (gItemType_dfgModoPInew.Type ()))
    {
        if (item.GetContext (scene))
            scene.EvalModInvalidate (SERVER_NAME_dfgModoPInew ".mod");
    }
}
...
Element::Element (CLxUser_Evaluation &eval, ILxUnknownID item_obj)
{
    ...
    _chan_index = eval.AddChan (item, CHN_NAME_INSTOBJ, LXfECHAN_WRITE);
...

{
  static int i = 0;
  char s[256];
  sprintf(s, "Element::Eval() beep! %ld", i++);
  feLog(s);
}
...
class Modifier : public CLxItemModifierServer
{
    public:
        static void initialize()
        {
            CLxExport_ItemModifierServer <Modifier> (SERVER_NAME_dfgModoPInew ".mod");
        }
...
const char * Modifier::ItemType ()
{
    ...
    return SERVER_NAME_dfgModoPInew;
}
...
CLxItemModifierElement * Modifier::Alloc (CLxUser_Evaluation &eval, ILxUnknownID item)
{
  ...
  Instance *GetInstance(ILxUnknownID item_obj)
  {
    ...
    // check the type.
    {
      ...
      const unsigned int numBytes = __min(strlen(typeName), strlen(SERVER_NAME_dfgModoPInew));
      if (memcmp(typeName, SERVER_NAME_dfgModoPInew, numBytes))
        return NULL;
    }
    ...
    if (pkg_inst.test())
    {
      CLxSpawner <Instance> spawn(SERVER_NAME_dfgModoPInew ".inst");
